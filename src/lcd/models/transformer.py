...
class ActionTransformer(nn.Module):
    def __init__(
        ...
        self.position_embeddings = nn.Embedding(
            max_position_embeddings, self.in_features
        )
        decoder_layer = nn.TransformerDecoderLayer(
            self.in_features,
            num_heads,
            activation="gelu",
            dim_feedforward=decoder_hidden_size,
            dropout=dropout_p,
            batch_first=True,
        )
        ...
        self.fc = nn.Sequential(
            nn.Linear(in_features=self.in_features, out_features=fc_hidden_size),
            nn.ReLU(),
            nn.Linear(in_features=fc_hidden_size, out_features=out_features),
        )

    def forward(
        self, perceptual_emb: torch.Tensor, language_emb: torch.Tensor
    ) -> torch.Tensor:
        if len(perceptual_emb.shape) == 2:
            perceptual_emb = perceptual_emb[
                :, None, :
            ]  # add dimension to make seq_len = 1

        batch_size, seq_len, in_features = perceptual_emb.shape
        assert (
            seq_len <= self.max_position_embeddings
        ), f"Maximum supported ctx length is {self.max_position_embeddings}, you passed in {seq_len}"
